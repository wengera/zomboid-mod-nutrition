...
| `exp02-20260910-030433` | `lifecycle.json` | `testing/experiments/s02_lifecycle.py` | [`docs/vanilla/food-item-model.md`](../../docs/vanilla/food-item-model.md); slice 13 [`docs/modding/wall-map.md`](../../docs/modding/wall-map.md) |
| `exp03-20260910-045523` | `body.json` | `testing/experiments/s03_body.py` | slice 03 (`.superpowers/sdd/03-body-side/task-3-report.md`; docs to follow); slice 13 [`docs/modding/wall-map.md`](../../docs/modding/wall-map.md) |
...
four facts recorded in the file itself. One disclosure that is *not* skew but belongs beside it:
a later residuals commit (`0449fa3`) touched only a **comment block** in `PZTestKit_Core.lua`
(and prose elsewhere) — **no executable path moved**, so the harness this file was measured on
is the harness at `291f977` in every sense that can change a number. It is nevertheless the artifact the next paragraph is
...
| `getHeat` on the client (frozen at 1.84703 across 12 s) | — | A real reading, and **one item over 12 s**. It says the client's copy of *this* server-spawned item did not tick; it is not a general law about client-side item updates. Quote it as the observation. **Dated note, 2026-09-11 (slice 11):** the "mechanism unexplained" half is no longer true for the cooking case. Slice 10 (`td2-20260910-231655`, `appendix.A2`) read a server-pinned `Base.Steak` that **did** move on the client, which put the canonical row in `docs/modding/patterns.md` § Measured MP sync facts into contention; slice 11 then ran the probe that row named — the same pin at `heat 1.2`, **below** `Food.update`'s 1.6 cooking gate (`td3-20260911-001948`, `carrier`) — and the client's copy was bit-frozen while the server's decayed. That row now reads **resolved per arm**: the carrier is the per-game-minute `sendItemStats` inside `Food.update`'s cooking branch, and **this** reading is its below-gate arm. **M per arm**, n = 1 in each; mechanism **C**. |
...
| `mod_log_lines_server`, read as the server's whole mod-log census | five lines: `loading simpleStatus` and four `overrides` lines (`ch`, `cn`, `de`, `en`) | **A SATURATED grep, not an exhaustive one.** The driver called `grep_file(server.log_path, MOD_RX, limit=5)` and `grep_file` **breaks** at its limit, so a result of exactly `limit` means the log had at least that many and the reader stopped. The server log itself holds **18** matching lines: `loading simpleStatus` (`:94`), **13** `overrides` lines at `:95-107` — one per shipped language, `ch` `cn` `de` `en` `es` `fr` `hu` `it` `ko` `pl` `pt` `ru` `tr` — and four `NoSuchFileException` lines for the `AnimSets` / `actiongroups` folders the mod does not ship (`:1278`, `:1299`, `:1320`, `:1341`, baseline noise). Quote **13**, from the log; the conclusion the four were used for (the mod shadows only its own JSONs, no key collision) is unchanged. Re-read 2026-09-11; the general rule is in `docs/testing/README.md` beside `grep_file`. |
| `summary.client_copy_ticked` | `true` | **It names the wrong question.** What `appendix.A2` measured is that the two VALUES moved and stayed bit-identical across a 0.5 s read offset; whether the client's copy *simulates* is not something two agreeing reads can settle. Resolved elsewhere and the other way: below `Food.update`'s 1.6 cooking gate the copy is **bit-frozen** (`td3-20260911-001948`, `carrier`), and `Food.update`'s cooking-branch `sendItemStats` is the carrier that moved this one. Cite the two readings and the gate, never this boolean. |
...
- **`nilcall_lines` is a bounded negative — and the bound is WIDER than one line.** Zero
  `tried to call nil|HasTrait|getTypeString` on either side proves that the whole
  **window-build** path did not execute the `common/` copies: `ISCharacterCook:createChildren`
  calls `AutoCook.init` at `:17` and `createCookingModeCombo` **unconditionally** at `:23`, which
  between them reach `common/…/AutoCook.lua:39` (from `AutoCook:init`),
  `common/…/AutoCook_AutoCraftRecipes.lua:39` (inside `initAutoCraftRecipes` `:3-54`, from
  `AutoCook:init:53`) and `common/…/ISCharacterCook.lua:221` (inside `createCookingModeCombo`
  `:199-235`). It is load-bearing because the line before the first of them
  (`common/…/AutoCook.lua:38`, the modData write) demonstrably did execute on this fresh
  character, and `snapshots[0].client.census_player` times that path at `session_ready + 1.044 s`.
  **None of the four calls sits at file scope**, so "dying at file load" is the wrong model — the
  raise would land when the character-info window is built, inside `createPlayerData` at spawn.
  What it still says nothing about is `common/…/ISCharacterCook.lua:50`, which sits inside
  `prerender` and is unreachable without opening the Cook tab; nothing on the bus opens it.
...
| `carrier.gate`, quoted as the gate's **comparison** | "… gated at heat **>= 1.6**; this probe pins heat at 1.2, BELOW it." | The `>=` is the **driver's** wording, not the jar's. `Food.update @63-@71 L373` is `ldc_w 1.600000023841858; fcmpl; ifle`, so a heat of **exactly** 1.6 does **not** enter the branch: the gate is **strict**, `heat > 1.6f`, which is what every doc in the library quotes. The artifact string and the driver docstring that produced it (`testing/experiments/td3_autocook.py:540`) are both frozen and both say `>=`. Quote the jar. Nothing measured here turns on it — the pin at 1.2 is below the gate under either reading. |
...
- **Provenance — the mods.** Three `path =` mods from `testing/experiments/`, not workshop items:
  `TKX_ItemOverride` (A), `TKX_Nutrient` (B) and `TKX_EatHook` (C). They are installed only through
  a profile and never sit in the fixture.
- **The two headline readings are per-side facts, not wire facts.** `phases.M2.items` (verdict `verdicts.M2b`; a narrowed
