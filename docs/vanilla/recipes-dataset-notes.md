...
| Q6 | Do the scanned counts equal what the game loaded, and does the Python expansion equal `EvolvedRecipe.getPossibleItems()` per recipe? | **Yes to both.** craft 969 / evolved 63 / legacy 0 live, equal to the scan; ten craft recipes compared field for field (77 fields, 75 matched at run time — the two misses are one field, § Discrepancies row 3); the five evolved lists are **set-equal** to the dataset's ingredient lists, 0 extra and 0 missing on either side | **M** `exp06-20260910-112726` ([`recipes.json`](../../testing/artifacts/exp06-20260910-112726/recipes.json)) |
...
- A bracketed list splits on `;`; an entry may carry a per-alternative count (`2:Base.CottonBalls`,
  86 of them). `types` keeps the id and the count stays readable in the line's `raw`. Ev C.
...
| `mode:destroy` on a non-`ItemCount` line | the charge is rounded **up to the whole items `RemoveItem` deletes** (`UseItem @260 L66-67`) and the annihilated remainder is published as `delta.destroyWaste`, so both readings — "hunger points moved" and "food removed from the world" — are recoverable. 1 vanilla row (`UnpackCigarettes`, all zeroes) | C |
...
| the five aliases (`RicePot`/`RicePan` → `Rice`, `PastaPot`/`PastaPan` → `Pasta`, `Roasted Vegetables` → `Stir fry`) | really used: 5 key parts (`RicePan` ×3, `RicePot` ×1, `Roasted Vegetables` ×1). Unaliased, `RicePan:1` would reach 1 recipe instead of 4 | C |
...
- Both JSONs are **byte-stable**: two runs of the scanner produce identical files, asserted by two
  install-gated tests and re-checked at every change.
...
`UnpackCigarettes` is the one `destroyWaste` row (all zeroes).
...
| A recipe whose IO is a **drink** completes server-side too | an MP client never calls `DrinkFluid`; fluid nutrition is per litre and is excluded from these deltas for that reason | C — [food-dataset-notes.md](food-dataset-notes.md) § MP behaviour |
