...
**Tag grammar.** A writer types `[#0417]`; `claims_check.py --fix-tags` appends the register's grade, bound token and status when they are not settled-C-none: `[#0417/M]`, `[#0417/M/n=1]`, `[#0417/W]`, `[#0417/C/inference]`, `[#0417/open]`, `[#0417/unverified]`. Several tags: `[#0417/M, #0512]`. The register carries the pointer and the full bound; a page restates a bound in words where the reader needs it ("measured on the dedicated-server path only"). No other citation form appears in the three layers; a run id may appear in prose only inside a sentence that also carries a tag whose pointer names that run.
