...
| recipes / new items / script overrides | RCON `additem "<user>" "<Base.ModItem>" 1`, then server `item.update`/`item.set` on it (that is what fires the `OnCooked`-class hooks; `recipes.craft <ModRecipe>` only **reads** the script and executes nothing) | server `item.get` vs client `witness.fields item <id> …` |
