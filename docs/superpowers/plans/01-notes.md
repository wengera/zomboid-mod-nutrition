...
| MP client | macro drain + `updateCalories` | skipped when `GameClient.client`; `updateWeight()` still runs — **but (slice 03) the client computes the weight and discards it**: a second `GameClient.client` skip at `updateWeight @317–@320 L198` sits before `setWeight` and before `applyTraitFromWeight`, so no weight and no band trait is written client-side (`docs/vanilla/body-stats.md` § MP behaviour, row 14, measured) | `Nutrition.update @42 L75`, `@106 L81`; `updateWeight @317–@320 L198` | C |
