...
- Claims health drains `22%` per in-game hour at Dying of Thirst, and 10–70 % reductions in "heat dissipation" per level. Slice 03 found the severe-moodle health loss on a `HUNGRY == 4` branch only (`healthReductionFromSevereBadMoodles / 50` = 3.3e-4 per multiplier unit, and per frame-normalised tick rather than per game-hour), `THIRST == 4` appearing only where the sleeping health addition is zeroed, and no thermoregulator read of `THIRST` anywhere. Verify against `BodyDamage.Update @1134-@1172` and `Thermoregulator`.
