...
- Silent on the nutrition side, which is where the mod's money is: `addItem` also sums `ingredientMacro x (1 + cookLvl/15) x share` into the dish, so a level-10 cook banks **1.1667x** the calories a level-0 cook does for the same dish hunger (measured, `exp02-20260910-030433`; Salad 25.0 -> 29.1667 kcal). The 1.6667x that `skillBonus` alone suggests is the **dish-gain / ingredient-loss** ratio, not the dish-size factor: `skillBonus` is cancelled by the `(1 - 0.03*cookLvl)` shrink inside `share`. See `docs/vanilla/food-item-model.md` § Evolved recipes.
